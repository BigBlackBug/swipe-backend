...
from swipe.matchmaking.matchmaker import Matchmaker
from swipe.matchmaking.schemas import MMSettings, MMRoundData
from swipe.swipe_server.users.enums import Gender


# def test_mm_time(mocker: MockerFixture):
#     user_service_mock = mocker.patch(
#         'swipe.matchmaking.matchmaker.MMUserService')
#     mm = Matchmaker(user_service_mock)
#     string_pool = set(secrets.token_hex(6) for _ in range(500))
#     connections = {
#         item: string_pool.difference(item) for item in string_pool
#     }
#     round_data = MMRoundData()
#     for user in connections.keys():
#         # same weight for everyone, like it's the first round
#         round_data.connect(user, MMSettings(age=10, gender=Gender.MALE,
#                                             gender_filter=Gender.MALE),
#                            connections=connections[user])
#
#     # 2 - build graph
#     mm.prepare_round(round_data)
#     assert False
#
...
#     for user in connections.keys():
#         # same weight for everyone, like it's the first round
#         round_data.connect(user, MMSettings(age=0, gender=Gender.MALE),
#                            connections=connections[user])
#
#     # 2 - build graph
#     mm.prepare_round(round_data)
